...
export function SidebarFlyoutProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarFlyoutContext.Provider value={{ showFlyout, hideFlyout }}>
      ...
      {flyout && createPortal(
        <div
          className="sidebar-flyout sidebar-flyout-enter glass-panel z-50 px-2.5 py-1 rounded-lg text-xs font-semibold text-primary whitespace-nowrap shadow-lg pointer-events-none border-l-2 border-l-accent-blue/40"
          style={{
            position: 'fixed',
            top: flyout.top,
            left: flyout.left,
