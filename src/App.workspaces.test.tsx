...
describe('App workspaces', () => {
  ...
  it('rejects empty flashcard sides with one message and persists trimmed values', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'New card' }))
    await user.click(screen.getByRole('button', { name: 'Save' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('Enter the front of the flashcard.')
    expect(screen.getByLabelText('Front')).toHaveAttribute('aria-invalid', 'true')
    expect(screen.getByLabelText('Front')).toHaveAttribute('aria-describedby', 'flashcard-front-error')
    expect(createSpy).not.toHaveBeenCalled()

    await user.type(screen.getByLabelText('Front'), '  Only front  ')
    await user.click(screen.getByRole('button', { name: 'Save' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('Enter the back of the flashcard.')
    expect(screen.getByLabelText('Back')).toHaveAttribute('aria-invalid', 'true')
    expect(screen.getByLabelText('Back')).toHaveAttribute('aria-describedby', 'flashcard-back-error')
    expect(createSpy).not.toHaveBeenCalled()
