...
/**
 * Persist a new goal. Owns id and created/updated timestamps.
 * When metric is study_time and period is daily, atomically syncs `dailyGoalMinutes` to `target`.
 */
export async function createGoal(fields: GoalWriteFields): Promise<StudyGoal> {
...
/**
 * Update an existing goal's editable fields and refresh `updatedAt`.
 * When metric is study_time and period is daily, atomically syncs `dailyGoalMinutes` to `target`.
 * Throws when no row matches `id`.
 */
export async function updateGoal(id: string, fields: GoalWriteFields): Promise<void> {
  const changes = {
    title: fields.title,
    target: fields.target,
    progress: fields.progress,
...
/**
 * Delete a goal by id. Missing rows are not treated as errors (Dexie delete is idempotent).
