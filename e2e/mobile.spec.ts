...
test('mobile timer has play/pause icon and extend/complete when running', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Extend timer by five minutes' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Complete current session' })).toBeVisible()
